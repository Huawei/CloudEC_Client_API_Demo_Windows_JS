...
#define MyAppName "TSDK"
#define MyAppVersion "6.1.0"
#define MyAppPublisher "Huawei Inc."
#define MyAppCopyright "Copyright (c) Huawei Technologies Co., Ltd. 2010-2018. All rights reserved."
#define MyAppExeName "tsdk_service_tray.exe"
...
#define StopTSDKDeamon "stopTSDKDeamon.bat"
#define TSDKDeamon "TSDKDeamon.bat"
...
procedure ExecStopTSDKDeamon();
var
    ResultCode: Integer;
begin
   Exec(ExpandConstant('{app}\stopTSDKDeamon.bat') ,'','',SW_HIDE, ewWaitUntilTerminated, ResultCode);   
end;
...
begin
sPrevPath := '';
if not RegQueryStringValue(HKCU,'Software\Microsoft\Windows\CurrentVersion\Uninstall\TSDK','UnistallString',sPrevPath) then
  RegQueryStringValue(HKLM,'SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\TSDK','UnistallString',sPrevPath);
  Result := (sPrevPath <> '');
end;
...
begin
  ...
  try
       newfilepathname := ExpandConstant('{userappdata}\TSDK\log\setup\');
  except
...
begin
  OverwriteFlag := false;
  if RegQueryDWordValue(HKCU, 'Software\Microsoft\Windows\CurrentVersion\Uninstall\TSDK',
...
if MsgBox(ExpandConstant('{cm:appDeleteConfig}'), mbConfirmation, MB_YESNO) = IDYES then
   ...
   DelTree(ExpandConstant('{localappdata}\TSDK'), True, True, True);
   DelTree(ExpandConstant('{userappdata}\TSDK'), True, True, True);
   end;
...
[InstallDelete]
;Name: {app}\TSDK; Type: filesandordirs
...
;Deutsch.appIsRunUninstall = TSDK wird ausgeführt. Die Anwendung wird geschlossen, wenn Sie die Deinstallation fortsetzen.
Chinese.appIsRunUninstall = TSDK 正在运行,继续卸载将关闭应用。
English.appIsRunUninstall = TSDK is running, continue to uninstall will close the application.
;Spanish.appDeleteConfig = ?Eliminar toda la información de usuario del teléfono virtual?
;Deutsch.appDeleteConfig = Alle TSDK-Benutzerinformationen l?schen?
English.appDeleteConfig = Are your shure delete user config info.
Chinese.appDeleteConfig = 是否要完全去除TSDK 用户信息。
;Spanish.appIsRunInstall = El teléfono virtual está en funcionamiento. La aplicación se cerrará si la instalación continúa.
;Deutsch.appIsRunInstall = TSDK wird ausgeführt. Die Anwendung wird geschlossen, wenn Sie die Installation fortsetzen.
English.appIsRunInstall = TSDK is running, continue installation will close the application.
Chinese.appIsRunInstall = TSDK 正在运行,继续安装将关闭应用。
...
;Spanish.appOverwrite = Whether to overwrite the installed TSDK
;Deutsch.appOverwrite = Whether to overwrite the installed TSDK
English.appOverwrite = Whether to overwrite the installed TSDK
Chinese.appOverwrite = 已经安装了TSDK，是否要覆盖安装？
;Spanish.appInstallSmall = Whether to use old version overwrite the installed TSDK
;Deutsch.appInstallSmall = Whether to use old version overwrite the installed TSDK
English.appInstallSmall = Whether to use old version overwrite the installed TSDK
...
[Registry]
Root:HKLM; Subkey:"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\TSDK";Flags: uninsdeletekeyifempty
Root:HKLM; Subkey:"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\TSDK"; ValueType: string; ValueName: "UnistallString"; ValueData: "{app}";Flags:uninsdeletekey
Root:HKCU; Subkey:"Software\Microsoft\Windows\CurrentVersion\Uninstall\TSDK";Flags: uninsdeletekeyifempty
Root:HKCU; Subkey:"Software\Microsoft\Windows\CurrentVersion\Uninstall\TSDK"; ValueType: string; ValueName: "UnistallString"; ValueData: "{app}";Flags:uninsdeletekey
Root:HKLM; Subkey:"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\TSDK";Flags: uninsdeletekeyifempty
Root:HKLM; Subkey:"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\TSDK"; ValueType: dword; ValueName: "InstalledVersion"; ValueData:  "{#MyCurVersion}";Flags:uninsdeletekey
Root:HKCU; Subkey:"Software\Microsoft\Windows\CurrentVersion\Uninstall\TSDK";Flags: uninsdeletekeyifempty
Root:HKCU; Subkey:"Software\Microsoft\Windows\CurrentVersion\Uninstall\TSDK"; ValueType: dword; ValueName: "InstalledVersion"; ValueData: "{#MyCurVersion}";Flags:uninsdeletekey
Root:HKCU; Subkey:"Software\Microsoft\Windows\Windows Error Reporting\ExcludedApplications";Flags: uninsdeletekeyifempty
Root:HKCU; Subkey:"Software\Microsoft\Windows\Windows Error Reporting\ExcludedApplications"; ValueType: dword; ValueName: "tsdk_service_tray.exe"; ValueData: 1;Flags:uninsdeletekey
Root:HKCU; Subkey:"Software\Classes\TSDKLaunch\DefaultIcon"; ValueType: string;  ValueName: ""; ValueData: "TSDKLaunch.exe,1";Flags:uninsdeletekey
Root:HKCU; Subkey:"Software\Classes\TSDKLaunch\shell\open\command"; ValueType: string; ValueName: ""; ValueData: """{app}\tsdk_service_tray.exe"" ""%1""";Flags:uninsdeletekey
Root:HKCR; SubKey:"TSDK"; ValueType: string; ValueData: "TSDK protocol";  Flags: CreateValueIfDoesntExist UninsDeleteKey;
Root:HKCR; SubKey:"TSDK"; ValueType: string; ValueName: "URL Protocol"; ValueData: "{app}\tsdk_service_tray.exe"; Flags: CreateValueIfDoesntExist;
Root:HKCR; SubKey:"TSDK\DefaultIcon"; ValueType: string; ValueData: "{app}\tsdk_service_tray.exe"; Flags: CreateValueIfDoesntExist;
Root:HKCR; SubKey:"TSDK\shell\open\command"; ValueType: string; ValueData: """{app}\tsdk_service_tray.exe"" ""%1"""; Flags: CreateValueIfDoesntExist;
...
Name: "{userstartup}\{#TSDKDeamon}"; Filename: "{app}\{#TSDKDeamon}";  Tasks: startmenu
...
[INI]  
Filename: "{userappdata}\TSDK\log\language.ini"; Section: "set"; Key: "lan"; String: "{code:MyLangName|{app}}"; Flags: uninsdeletesection;
Filename: "{userappdata}\TSDK\log\version.ini"; Section: "BuildTime"; Key: "time"; String: "{#MyDateTimeString}"; Flags: uninsdeletesection;
Filename: "{userappdata}\TSDK\log\version.ini"; Section: "Version"; Key: "version"; String: "{#MyAppVersion}"; Flags: uninsdeletesection;
